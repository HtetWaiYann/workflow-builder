...
- **backend** — Express 4 REST API. Runs with `ts-node-dev` in dev (CommonJS, `module: "CommonJS"`). Imports from `@triggr/shared` via the workspace symlink at `node_modules/@triggr/shared → ../../packages/shared`.
...
- Always check if a type already exists in @triggr/shared before creating a new one
...
- Every node executor must implement the NodeExecutor interface from @triggr/shared

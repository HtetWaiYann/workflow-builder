...
// POST /workspace/variables — Creates a new encrypted workspace variable.
variablesRouter.post('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const variable = await prisma.workspaceVariable.create({
      data: { workspaceId: req.workspaceId!, key, encryptedValue, iv, authTag },
      select: { id: true, key: true, createdAt: true, updatedAt: true },
    })
...
// PUT /workspace/variables/:variableId — Updates the encrypted value of an existing variable.
variablesRouter.put('/:variableId', async (req: AuthRequest, res: Response) => {
  try {
...
// DELETE /workspace/variables/:variableId — Permanently removes a workspace variable.
